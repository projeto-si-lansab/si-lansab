...
**
** Date of generation: 2014-11-22T22:31:43
** Command line:  $ uaadaptor.exe -n 'C:\Program Files (x86)\Esterel Technologies\Esterel SCADE 6.4.2\SCADE Display\config\Workspace\environments\UAPageCreator\A661Norm\a661.xml' -o 'ua' -outdir '..\UserApplication\KCG\' -k '..\UserApplication\KCG\kcg_trace.xml -d '..\DefinitionFile\INTEGRATION_TS03DF\TS03.sgfx' ..\UserApplication\TS03.sdy
**/
...
A661_INTERNAL int a661_com_msg_GpLinePolar_A661_ROTATION_ANGLE(buffer_el msg[], 
...
A661_INTERNAL int a661_com_csp_widget_GpRectangle(buffer_el msg[], 
  ...
  a661_uchar A661_FILL_INDEX_FillIndex_,  a661_bool A661_SIZE_X__emit__, 
...
A661_INTERNAL int a661_com_csp_widget_GpLinePolar(buffer_el msg[], 
  ...
  if (A661_ROTATION_ANGLE__emit__){
    ...
    a661_msg_csp_header(msg, wid);
    if (A661_ROTATION_ANGLE__emit__){
      n1 = a661_com_msg_GpLinePolar_A661_ROTATION_ANGLE(msg + n0, 46096,
...
A661_INTERNAL int a661_com_A661_CMD_UA_REQUEST_layer_1(buffer_el msg[], 
  outC_TS03* scade_ctx,  extern_out_ctx_t * extern_ctx){
  int n2 = 0;
  int n1 = 0;
  if (scade_ctx->Layer1Vixible ||
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_TS03* scade_ctx, 
  ...
  scade_ctx->Layer1Active ||
  scade_ctx->Layer1Vixible){
    LAYER("ts03#1");
    n0 = 8 /*include block_size*/;
    a661_msg_cmd_header(msg, 1, DEFAULT_CONTEXT_NUMBER);
    if (scade_ctx->Emit2CDS){
      WIDGET("GpLinePolar_Battery#2055");
      n1 = a661_com_csp_widget_GpLinePolar(msg + n0, 2055, scade_ctx->Emit2CDS,
        scade_ctx->ST_BatteryLevel);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("GpRectangle_BatteryWarn#2500");
      n2 = a661_com_csp_widget_GpRectangle(msg + n0 + n1, 2500,
        scade_ctx->Emit2CDS, scade_ctx->BatteryWarn, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("GpArcCircleCameraOn#2700");
      n3 = a661_com_csp_widget_GpArcCircle(msg + n0 + n1 + n2, 2700,
        scade_ctx->Emit2CDS, scade_ctx->ST_CameraOnIndicator);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("GpRectangle_MemoryWarn#3200");
      n4 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3, 3200,
        scade_ctx->Emit2CDS, scade_ctx->MemoryWarn, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("GpArcCircleDownloadOn#3400");
      n5 = a661_com_csp_widget_GpArcCircle(msg + n0 + n1 + n2 + n3 + n4, 3400,
        scade_ctx->Emit2CDS, scade_ctx->ST_MemoryDownloadIndicator);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("GpRectangleMemoryGauge#3550");
      n6 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5,
        3550, 0, 0, scade_ctx->Emit2CDS, scade_ctx->ST_MemoryGauge);
    }
    if (scade_ctx->Layer1Active ||
    scade_ctx->Layer1Vixible){
      n7 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6,
        scade_ctx, extern_ctx);
    }
    n8 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7, A661_END_BLOCK);
    size = n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 ;
}
 int ua_send(buffer_el msg[],  outC_TS03* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_TS03* scade_ctx,  extern_in_ctx_t * extern_ctx){
  scade_ctx->TST_TakePicutreBtn = FALSE;
  scade_ctx->ST_DownloadBtn = FALSE;
  scade_ctx->ST_CameraBtnOn = FALSE;
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_TS03* scade_ctx, 
                  ...
                  case 3055 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        ...
                        /*  Notify */
                        scade_ctx->TST_TakePicutreBtn = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
