...
**
** Date of generation: 2014-11-19T23:22:40
** Command line:  $ uaadaptor.exe -n 'C:\Program Files\Esterel Technologies\Esterel SCADE 6.4.2\SCADE Display\config\Workspace\environments\UAPageCreator\A661Norm\a661.xml' -o 'ua' -outdir '..\UserApplication\KCG\' -k '..\UserApplication\KCG\kcg_trace.xml -d '..\DefinitionFile\INTEGRATION_DisplayTS01DF\DisplayTS01.sgfx' ..\UserApplication\TS01.sdy
**/
...
A661_INTERNAL int a661_com_msg_BlinkingContainer_A661_VISIBLE(buffer_el msg[], 
...
A661_INTERNAL int a661_com_msg_GpRectangle_A661_SIZE_Y(buffer_el msg[], 
  a661_ushort pid,  a661_ulong A661_SIZE_Y_SizeY_){
  int n3 = 0;
  int n2 = 0;
  int n1 = 0;
  PARAMETER("A661_SIZE_Y");
  WRITE_USHORT(msg + 0, pid);
  WRITE_USHORT(msg + 2, 0);
...
A661_INTERNAL int a661_com_csp_widget_Label(buffer_el msg[],  a661_ushort wid, 
  ...
  unsigned char * A661_STRING_LabelString_, 
  a661_ushort A661_STRING_String_size_,  a661_bool A661_VISIBLE__emit__, 
  ...
  A661_STRING__emit__){
    ...
    if (A661_STRING__emit__){
      n2 = a661_com_msg_Label_A661_STRING(msg + n0 + n1, 46224,
...
A661_INTERNAL int a661_com_csp_widget_GpRectangle(buffer_el msg[], 
  ...
  a661_uchar A661_FILL_INDEX_FillIndex_,  a661_bool A661_SIZE_Y__emit__, 
  a661_ulong A661_SIZE_Y_SizeY_){
  int size = 0;
  int n2 = 0;
  int n1 = 0;
  int n0 = 0;
  ...
  A661_FILL_INDEX__emit__){
    ...
    a661_msg_csp_header(msg, wid);
    if (A661_SIZE_Y__emit__){
      n1 = a661_com_msg_GpRectangle_A661_SIZE_Y(msg + n0, 46176,
        A661_SIZE_Y_SizeY_);
    }
    if (A661_FILL_INDEX__emit__){
      n2 = a661_com_msg_GpRectangle_A661_FILL_INDEX(msg + n0 + n1, 45536,
...
A661_INTERNAL int a661_com_csp_widget_BlinkingContainer(buffer_el msg[], 
  a661_ushort wid,  a661_bool A661_VISIBLE__emit__, 
  a661_bool A661_VISIBLE_Visible_){
  int size = 0;
  int n1 = 0;
  int n0 = 0;
  if (A661_VISIBLE__emit__){
    ...
    a661_msg_csp_header(msg, wid);
    if (A661_VISIBLE__emit__){
      n1 = a661_com_msg_BlinkingContainer_A661_VISIBLE(msg + n0, 46384,
        A661_VISIBLE_Visible_);
    }
    size = n0 + n1;
    WRITE_USHORT(msg + 2, size);
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_Main* scade_ctx, 
  ...
  scade_ctx->Layer1Visible){
    ...
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_PhF#11");
      n1 = a661_com_csp_widget_GpRectangle(msg + n0, 11, scade_ctx->Emit2CDS,
        scade_ctx->PhF_Color, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph1#12");
      n2 = a661_com_csp_widget_GpRectangle(msg + n0 + n1, 12,
        scade_ctx->Emit2CDS, scade_ctx->Ph1_Color, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Fuel_Level#13");
      n3 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2, 13, 0, 0,
        scade_ctx->Emit2CDS, scade_ctx->Fuel_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph3#14");
      n4 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3, 14,
        scade_ctx->Emit2CDS, scade_ctx->Ph3_Color, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph2#15");
      n5 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4, 15,
        scade_ctx->Emit2CDS, scade_ctx->Ph2_Color, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("EBN_Latitude#26");
      n6 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5,
        26, scade_ctx->Emit2CDS, scade_ctx->Latitude_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("EBN_Longitude#27");
      n7 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6,
        27, scade_ctx->Emit2CDS, scade_ctx->Longitude_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("EBN_Inclination#28");
      n8 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7,
        28, scade_ctx->Emit2CDS, scade_ctx->Inclination_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_Automatic#32");
      n9 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8,
        32, 0, 0, 0, 0, scade_ctx->Emit2CDS, scade_ctx->Auto_Txt_En);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_Manual#33");
      n10 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9,
        33, 0, 0, 0, 0, scade_ctx->Emit2CDS, scade_ctx->Manual_Txt_En);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_Ph1#43");
      n11 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10,
        43, scade_ctx->Emit2CDS, 20, scade_ctx->Ph1_Label,
        scade_ctx->StringSize, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_Ph2#44");
      n12 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11,
        44, scade_ctx->Emit2CDS, 20, scade_ctx->Ph2_Label,
        scade_ctx->StringSize, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_Ph3#45");
      n13 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12,
        45, scade_ctx->Emit2CDS, 20, scade_ctx->Ph3_Label,
        scade_ctx->StringSize, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Lab_PhF#46");
      n14 = a661_com_csp_widget_Label(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13,
        46, scade_ctx->Emit2CDS, 20, scade_ctx->PhF_Label,
        scade_ctx->StringSize, 0, 0);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("EBN_Speed#49");
      n15 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14,
        49, scade_ctx->Emit2CDS, scade_ctx->Speed_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("EBN_Altitude#255");
      n16 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15,
        255, scade_ctx->Emit2CDS, scade_ctx->Altitude_Out);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Destroyed_BlinkingContainer#275");
      n17 = a661_com_csp_widget_BlinkingContainer(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16,
        275, scade_ctx->Emit2CDS, scade_ctx->Destroyed_Label);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("BlinkingContainer#277");
      n18 = a661_com_csp_widget_BlinkingContainer(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17,
        277, scade_ctx->Emit2CDS, scade_ctx->SatLaunched);
    }
    if (scade_ctx->Layer1Active ||
    scade_ctx->Layer1Visible){
      n19 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18,
        scade_ctx, extern_ctx);
    }
    n20 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19,
      A661_END_BLOCK);
    size = n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20 ;
}
 int ua_send(buffer_el msg[],  outC_Main* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_Main* scade_ctx,  extern_in_ctx_t * extern_ctx){
  
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_Main* scade_ctx, 
