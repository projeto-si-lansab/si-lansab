...
**
** Date of generation: 2014-10-24T11:03:21
** Command line:  $ uaadaptor.exe -n 'C:\Arquivos de programas\Esterel Technologies\Esterel SCADE 6.4.2\SCADE Display\config\Workspace\environments\UAPageCreator\A661Norm\a661.xml' -o 'ua' -outdir '..\UserApplication\KCG\' -k '..\UserApplication\KCG\kcg_trace.xml -d '..\DefinitionFile\INTEGRATION_ts04DF\ts04.sgfx' ..\UserApplication\ts04.sdy
**/
...
A661_INTERNAL int a661_com_msg_GpRectangle_A661_FILL_INDEX(buffer_el msg[], 
...
A661_INTERNAL int a661_com_csp_widget_PushButton(buffer_el msg[], 
  ...
  a661_uchar A661_ENABLE_Enable_){
  int size = 0;
  int n1 = 0;
  int n0 = 0;
  if (A661_ENABLE__emit__){
    ...
    if (A661_ENABLE__emit__){
      n1 = a661_com_msg_PushButton_A661_ENABLE(msg + n0, 45440,
        A661_ENABLE_Enable_);
    }
    size = n0 + n1;
    WRITE_USHORT(msg + 2, size);
...
A661_INTERNAL int a661_com_csp_widget_GpRectangle(buffer_el msg[], 
  ...
  if (A661_FILL_INDEX__emit__){
    ...
    a661_msg_csp_header(msg, wid);
    if (A661_FILL_INDEX__emit__){
      n1 = a661_com_msg_GpRectangle_A661_FILL_INDEX(msg + n0, 45536,
...
A661_INTERNAL int a661_com_A661_CMD_UA_REQUEST_layer_1(buffer_el msg[], 
  outC_ts04* scade_ctx,  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_ts04* scade_ctx, 
  ...
  scade_ctx->LayerVisible){
    ...
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_PhF#5");
      n1 = a661_com_csp_widget_GpRectangle(msg + n0, 5, scade_ctx->Emit2CDS,
        scade_ctx->color_fuel4);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph1#6");
      n2 = a661_com_csp_widget_GpRectangle(msg + n0 + n1, 6,
        scade_ctx->Emit2CDS, scade_ctx->color_fuel1);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph3#8");
      n3 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2, 8,
        scade_ctx->Emit2CDS, scade_ctx->color_fuel3);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph2#9");
      n4 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3, 9,
        scade_ctx->Emit2CDS, scade_ctx->color_fuel2);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_PhF#17");
      n5 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4, 17,
        scade_ctx->Emit2CDS, scade_ctx->Estagio4CorFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph1#18");
      n6 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5,
        18, scade_ctx->Emit2CDS, scade_ctx->Estagio1CorFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph3#19");
      n7 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6,
        19, scade_ctx->Emit2CDS, scade_ctx->Estagio3CorFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Ph2#20");
      n8 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7,
        20, scade_ctx->Emit2CDS, scade_ctx->Estagio2CorFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("BoxCountVal#36");
      n9 = a661_com_csp_widget_EditBoxNumeric(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8,
        36, scade_ctx->Emit2CDS, scade_ctx->ValContagem);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Computador#42");
      n10 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9,
        42, scade_ctx->Emit2CDS, scade_ctx->CorComputadorFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("Rec_Camera#43");
      n11 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10,
        43, scade_ctx->Emit2CDS, scade_ctx->CorCameraFundo);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("AlertaGeralRec#54");
      n12 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11,
        54, scade_ctx->Emit2CDS, scade_ctx->CorAlertaGeral);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh1Rectangle#66");
      n13 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12,
        66, scade_ctx->Emit2CDS, scade_ctx->CorPh1);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh2Rectangle#68");
      n14 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13,
        68, scade_ctx->Emit2CDS, scade_ctx->CorPh2);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh3Rectangle#69");
      n15 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14,
        69, scade_ctx->Emit2CDS, scade_ctx->CorPh3);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh4Rectangle#70");
      n16 = a661_com_csp_widget_GpRectangle(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15,
        70, scade_ctx->Emit2CDS, scade_ctx->CorPh4);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh1Button#74");
      n17 = a661_com_csp_widget_PushButton(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16,
        74, scade_ctx->Emit2CDS, scade_ctx->Ph1Enabled);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh2Button#75");
      n18 = a661_com_csp_widget_PushButton(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17,
        75, scade_ctx->Emit2CDS, scade_ctx->Ph2Enabled);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh3Button#76");
      n19 = a661_com_csp_widget_PushButton(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18,
        76, scade_ctx->Emit2CDS, scade_ctx->Ph3Enabled);
    }
    if (scade_ctx->Emit2CDS){
      WIDGET("desacoplarPh4Button#77");
      n20 = a661_com_csp_widget_PushButton(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19,
        77, scade_ctx->Emit2CDS, scade_ctx->Ph4Enabled);
    }
    if (scade_ctx->LayerVisible){
      n21 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20,
        scade_ctx, extern_ctx);
    }
    n22 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20 + n21,
      A661_END_BLOCK);
    size = n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20 + n21 + n22;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + n10 + n11 + n12 + n13 + n14 + n15 + n16 + n17 + n18 + n19 + n20 + n21 + n22 ;
}
 int ua_send(buffer_el msg[],  outC_ts04* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_ts04* scade_ctx,  extern_in_ctx_t * extern_ctx){
  scade_ctx->verify_fuel = FALSE;
  scade_ctx->VerificarEstagios = FALSE;
  scade_ctx->SelfDestructSignal = FALSE;
  scade_ctx->VerificarComputador = FALSE;
  scade_ctx->VerificarCamera = FALSE;
  scade_ctx->DesacoplarPh1 = FALSE;
  scade_ctx->DesacoplarPh2 = FALSE;
  scade_ctx->DesacoplarPh3 = FALSE;
  scade_ctx->DesacoplarPh4 = FALSE;
  scade_ctx->enviarComando = FALSE;
  scade_ctx->LancAutomatico = FALSE;
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_ts04* scade_ctx, 
                  ...
                  case 47 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        /*  PushButton_A661_EVT_SELECTION */
                        /*  Notify */
                        scade_ctx->VerificarCamera = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
                  ...
                  case 45 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        /*  PushButton_A661_EVT_SELECTION */
                        /*  Notify */
                        scade_ctx->VerificarComputador = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
                  ...
                  case 38 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        /*  PushButton_A661_EVT_SELECTION */
                        /*  Notify */
                        scade_ctx->SelfDestructSignal = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
                  ...
                  case 32 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_STATE_CHANGE : 
                        /*  ToggleButton_A661_EVT_STATE_CHANGE */
                        /*  EvtToggleState */
                        scade_ctx->ToogleIniciar = buffer[ ( 4 + n ) ];
                        break;
                      default : break;
                    }
                    n = next_n;
                  ...
                  case 28 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        /*  PushButton_A661_EVT_SELECTION */
                        /*  Notify */
                        scade_ctx->VerificarEstagios = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
